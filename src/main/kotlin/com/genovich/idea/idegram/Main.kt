...
class Main : ToolWindowFactory {
    ...
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        ...
        val content = toolWindow.contentManager.factory.createContent(panel, "Idegram", false)
    ...
    private fun handleApiObject(apiObject: TdApi.Object) {
        ...
        when (val tdApiObject = TdApiObject.parse(apiObject)
            ...
            is TdApiObject.Update -> {
                when (tdApiObject) {
                    is TdApiObject.Update.AuthorizationState -> when (val authState = tdApiObject.state) {
                        ...
                        TdApiObject.AuthorizationState.Ready -> {
                            replaceContent(MAIN_TAB_NAME) { row { label("loading chat list") } }
                        ...
                        TdApiObject.AuthorizationState.Closed -> TODO()
                        TdApiObject.AuthorizationState.Closing -> TODO()
                        TdApiObject.AuthorizationState.LoggingOut -> TODO()
                        is TdApiObject.AuthorizationState.WaitPassword -> replaceContent(
                            MAIN_TAB_NAME
                        ) {

                        }
                        is TdApiObject.AuthorizationState.WaitRegistration -> replaceContent(
                            MAIN_TAB_NAME
                        ) {
                            val firstNameField = JTextField()
                            val lastNameField = JTextField()
                            val acceptTermsOfService = JCheckBox("Accept terms of service")
                            row {
                                label("first name")
                                firstNameField()
                            }
                            ...
                            row {
                                JTextArea(authState.termsOfService.text.text)()
                            }
                            row {
                            ...
                            row {
                                button("submit") {
                                    if (acceptTermsOfService.isSelected) {
                                        client.send(TdApi.RegisterUser(firstNameField.text, lastNameField.text)) { }
                                    }
                                }
                            }
                        }
                        is TdApiObject.AuthorizationState.WaitOtherDeviceConfirmaiton -> TODO()
                    }
